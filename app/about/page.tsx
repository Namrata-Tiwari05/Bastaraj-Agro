...
const AboutPage = () => {
  ...
  return (
    <main className="w-full min-h-screen bg-[#0D1713] text-white antialiased overflow-hidden pb-20">
      ...
      <section className="relative w-full pt-12 pb-16 px-4 sm:px-6 lg:px-8 z-10 max-w-7xl mx-auto mt-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 space-y-6 text-center lg:text-left">
            ...
            <div className="grid grid-cols-3 gap-3 pt-4">
              {[
                ...
                <div key={i} className="bg-[#16261F]/85 border border-[#2C4538] p-3 rounded-2xl flex flex-col items-center justify-center shadow-[0_10px_40px_rgba(0,0,0,0.25)] hover:bg-[#1B2D25] hover:border-[#41A61D]/50 transition-all duration-300">
                  <metric.icon className="text-[#41A61D] text-lg mb-1" />
                  <span className="text-sm font-black text-white tracking-tight text-center leading-none">{metric.label}</span>
                  <span className="text-[9px] font-bold text-[#8EA39A] uppercase mt-1 text-center">{metric.desc}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-7 w-full h-[320px] sm:h-[450px] relative rounded-[2.5rem] overflow-hidden shadow-2xl border border-[#2C4538] bg-[#16261F]/50">
            <Image 
              src="/img1.webp" 
      ...
      <section className="w-full py-12 px-4 sm:px-6 lg:px-8 z-10 relative max-w-7xl mx-auto">
        <div className="mb-8 text-center lg:text-left">
          <h2 className="text-xl font-black text-white uppercase tracking-widest flex items-center gap-2 justify-center lg:justify-start">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {showcasedPlants.map((plant, idx) => (
            <div key={idx} className="relative h-48 sm:h-64 rounded-[2rem] overflow-hidden border border-[#2C4538] shadow-[0_10px_40px_rgba(0,0,0,0.25)] bg-gray-950 group hover:border-[#41A61D]/50 transition-all duration-350">
              <Image 
                src={plant.path} 
                alt={plant.name} 
                fill 
                ...
                unoptimized={true}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/30 to-transparent opacity-90 z-10"></div>
              <div className="absolute bottom-4 left-4 right-4 z-20 flex flex-col">
                ...
                <span className="text-base font-black text-white tracking-tight mt-0.5 group-hover:text-[#41A61D] transition-colors">{plant.name}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="w-full py-10 px-4 sm:px-6 lg:px-8 z-10 relative max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex gap-4 items-center bg-[#16261F]/85 border border-[#2C4538] p-6 rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.25)] hover:bg-[#1B2D25] hover:border-[#41A61D]/50 transition-all duration-300">
            <div className="w-12 h-12 rounded-2xl bg-[#41A61D]/10 text-[#41A61D] flex items-center justify-center shrink-0 border border-[#41A61D]/20 shadow-inner">
            ...
            <div>
              ...
              <p className="text-[#C6D1CC] text-sm font-bold tracking-tight mt-1 leading-snug">
      ...
      <section className="w-full py-10 px-4 sm:px-6 lg:px-8 z-10 relative max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {team.map((member, i) => (
            <div 
              key={i}
              className="bg-[#16261F]/85 border border-[#2C4538] p-4 rounded-2xl flex items-center justify-between shadow-[0_10px_40px_rgba(0,0,0,0.25)] hover:bg-[#1B2D25] hover:border-[#41A61D]/50 transition-all duration-300"
            >
              ...
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-white/5 to-[#41A61D]/10 border border-[#41A61D]/20 flex items-center justify-center shrink-0 font-black text-xs text-[#41A61D]">
                {member.initial}
              </div>
            </div>
          ))}

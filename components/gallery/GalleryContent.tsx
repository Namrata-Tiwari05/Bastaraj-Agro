...
const GalleryContent = () => {
  ...
  return (
    <main className="w-full min-h-screen bg-[#0D1713] text-white antialiased overflow-hidden pb-32">
      ...
      {/* Background Micro Grid Layer (White layout dots for clear look) */}
      ...
      <section className="relative w-full pt-12 pb-10 px-4 sm:px-6 lg:px-8 z-10 max-w-7xl mx-auto mt-6 text-center">
        <div className="space-y-4">
          ...
          <h1 className="text-3xl sm:text-5xl font-black text-white tracking-tight leading-none font-sans">
            Our Plantation <span className="text-[#41A61D] drop-shadow-[0_0_15px_rgba(65,166,29,0.3)]">Gallery</span>
          </h1>
          <p className="text-sm sm:text-base text-[#8EA39A] font-medium max-w-xl mx-auto leading-relaxed">
        ...
        <div className="flex overflow-x-auto gap-2 pb-2 justify-start sm:justify-center [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider border active:scale-95 transition-all whitespace-nowrap cursor-pointer ${
                activeFilter === cat
                  ? 'bg-[#41A61D] text-white border-[#41A61D] shadow-[0_4px_12px_rgba(65,166,29,0.3)]'
                  : 'bg-[#16261F]/85 text-[#8EA39A] border-[#2C4538] hover:bg-[#1B2D25] hover:text-white'
              }`}
            >
              {cat}
            </button>
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 z-10 relative max-w-7xl mx-auto">
        {filteredItems.length === 0 ? (
          <div className="w-full text-center py-20 bg-[#16261F]/85 border border-[#2C4538] rounded-[2rem] shadow-2xl">
            <p className="text-gray-450 text-sm font-bold">No media records found for this block.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map((item) => (
              <div 
                key={item.id}
                className="relative h-64 sm:h-72 rounded-[2rem] overflow-hidden border border-[#2C4538] shadow-2xl bg-[#16261F] group hover:border-[#41A61D]/50 transition-all duration-350"
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/35 to-transparent opacity-90 z-10"></div>
                
                {/* Floating Meta Labels on top right edge - Icon color uses Primary #41A61D */}
                <div className="absolute top-4 right-4 bg-[#16261F]/90 backdrop-blur-md border border-[#2C4538] px-2.5 py-1 rounded-lg text-[9px] font-black text-white tracking-wider uppercase shadow-sm flex items-center gap-1 z-20">
                  <FiCamera className="text-[#41A61D] animate-pulse" /> {item.category}
                </div>
